            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "Statistiques récupérées",
                            content = @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = ResponseStatsDto.class))),
                    @ApiResponse(responseCode = "4xx", description = "Erreur client"),
                    @ApiResponse(responseCode = "5xx", description = "Erreur serveur")
            })
    @PreAuthorize("hasAnyAuthority('SCOPE_ADMIN')")
    @GetMapping("/{id}/stats")
    public ResponseEntity<ResponseStatsDto> getStats(@PathVariable Integer id) {
        ResponseStatsDto stats = enseignantServiceImpl.getStats(id);
